...
public class Animator {
    ...
    public void endPhase() {
        ...
        if(currentPhase == null) { // Last phase was the main phase
            float totalDuration = prevPhase.getDuration();
            float tick = getTick(currentEntity) - prevPhase.getOffset();
            if(tick >= totalDuration && prevPhase.isLooping()) { // Loop main phase
                reset(currentEntity);
            }
        }
    }
     ...
     *          Set it to <code>true</code> if you want the rotation offset to stay at the same position; set it to
     *          <code>false</code> if you want to move with the part
     */
