    ...
    public List<Target> nextNavigatePositions()
    {
        ...
        for (Entity distEnt : distEnts)
        {
            ...
            if (!seek)
            {
                ...
                for (ItemStack dropItem: dropItemStacks)
                {
                    if (dropItemStacks != null && dropItemStacks.size() > 0 && this.demand.isItemDemanded(dropItem) != null)
                        newTargets.add(new Target(this.getAgent().worldObj, distEnt));
                }
                //seek = true;
            }

            if (newTargets != null && this.evaluateTarget(t))
                return newTargets;
        }
