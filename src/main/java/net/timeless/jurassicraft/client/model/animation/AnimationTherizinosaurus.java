...
public class AnimationTherizinosaurus implements IModelAnimator
{
    ...
    class CurrentPoseArray
    {
        ...
        {
            rotationArray = new float[parNumParts][3];
            positionArray = new float[parNumParts][3];
            offsetArray = new float[parNumParts][3];
        }
...
//        parModel.chainWave(bodyParts, 0.1F, -0.03F, 5, frame, 1F);
    ...
    protected void performJabelarAnimations(ModelDinosaur parModel, float f, float f1, float rotation, float rotationYaw, float rotationPitch, float partialTicks, EntityTherizinosaurus parEntity)
    {
        ...
        if (currentAnimation.get(parEntity.getEntityId()).finishedTween)
        {
    ...
    {
        CurrentAnimation theAnimation = currentAnimation.get(parEntity.getEntityId());
    ...
    {
        CurrentAnimation theAnimation = currentAnimation.get(parEntity.getEntityId());
        CurrentPoseArray theCurrentPose = currentPose.get(parEntity.getEntityId());
        MowzieModelRenderer[] theTargetPose = targetPose.get(parEntity.getEntityId());
...
//        System.out.println("finished tween");
        ...
        currentAnimation.get(parEntity.getEntityId()).incrementSequenceStep();

        currentAnimation.get(parEntity.getEntityId()).finishedTween = false;
        setNextTween(parEntity);
    }
